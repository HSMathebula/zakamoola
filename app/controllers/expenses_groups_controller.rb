class ExpensesGroupsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @expenses_group.save
        format.html do
          redirect_to expenses_group_url(@expenses_group), notice: 'Expenses group was successfully created.'
        end
  ...
  def update
    respond_to do |format|
      if @expenses_group.update(expenses_group_params)
        format.html do
          redirect_to expenses_group_url(@expenses_group), notice: 'Expenses group was successfully updated.'
        end
  ...
  def expenses_group_params
    params.require(:expenses_group).permit(:group_id, :expense_id)
  end
end
